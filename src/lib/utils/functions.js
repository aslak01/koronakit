...
export function computeMovingAverage(data, period, datekey, valuekey, avgkey) {
  ...
  const getAverage = (data, valuekey) =>
    data.reduce((acc, val) => acc + val[valuekey], 0) / data.length
  for (let x = 0; x + period - 1 < sortedData.length; x += 1) {
    let row = {}
    if (Array.isArray(valuekey)) {
      for (let i = 0; i < valuekey.length; i += 1) {
        row[valuekey[i]] = Math.round(
          getAverage(sortedData.slice(x, x + period), valuekey[i])
        )
      }
    } else {
      row[avgkey] = Math.round(
        getAverage(sortedData.slice(x, x + period), valuekey)
      )
    }
    row[datekey] =
      period > 1
        ? sortedData.slice(x, x + period)[Math.round(period / 2)][datekey]
        : sortedData.slice(x, x + period)[0][datekey]
