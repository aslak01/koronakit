<script>
  ...
  import { minidaySettings } from '$lib/utils/store.js'
  ...
  function prepData() {
    ...
    Object.values(data.cases).forEach((v) => {
      values.push(v)
    })
  }
  prepData()
  ...
  $: updShv = shavedData.map((v) => ({
    ...v,
    pmil: parseInt(insidens(v[yKey], worldPop))
  }))
  ...
  $: maxPm = Math.max.apply(
    Math,
    updShv.map((d) => d[pMilKey])
  )
