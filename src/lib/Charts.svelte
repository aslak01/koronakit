<script>
  ...
  import Minidays from '$lib/components/Minidays.svelte'
  ...
  $: $minidaySettings.aMax =
    maximum[0] != undefined ? Math.max.apply(Math, maximum) : undefined

  $: $minidaySettings.pMax =
    pMmax[0] != undefined ? Math.max.apply(Math, pMmax) : undefined
