<script>
  ...
  let starter =
    data.data.total.findIndex((n) => n.total > 0) / data.data.total.length
  let start = starter
  let end = 1
  ...
  $: lastReportShaved =
    shavedData[shavedData.length - 1].date.toLocaleDateString('no-NO')

  $: lastDay = new Intl.NumberFormat('no-NO').format(
    shavedData[shavedData.length - 1].total
  )
  $: lastDayNew = new Intl.NumberFormat('no-NO').format(
    shavedData[shavedData.length - 1].total -
      shavedData[shavedData.length - 2].total
  )
  ...
  <article class="chart">
    <div class="chart-container">
      ...
      >
        ...
        <Html>
          <SharedTooltip formatTitle={formatTickX} dataset={shavedData} />
        </Html>
      </LayerCake>
    </div>
  </article>
