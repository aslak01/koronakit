<script>
  ...
  const restligeDiag = [
    'Syst. meningokokksykdom',
    'Aids',
    'Paratyfoidfeber',
    'Tyfoidfeber',
    'Syst. pneumokokksykdom',
    'Virale infeksjoner i sentralnervesystemet',
    'Syst. gr. B streptokokksykdom',
    'Syst. gr. A streptokokksykdom',
    'Prionsykdommer',
    'Alvorlige importsykdommer',
    'Alvorlige miljøsykdommer',
    'Mat- og vannbårne sykdommer',
    'Sykdommer forårsaket av visse resistente mikrober',
  ...
  var sumobjects = data.map((y) =>
    Object.keys(y)
      .filter((k) => k !== ignored.some((p) => p === k))
      .reduce((x, z) => x + y[z], 0)
  )
    ...
    if (data[index].År < 2020) {
      summedData.push({
        ...
        'Seksuelt overførbare sykdommer':
          data[index]['Seksuelt overførbare sykdommer']
      })
...
<article class="text">
  ...
  <p>
    Data fra <a href="https://statistikk.fhi.no/msis"
      >det nasjonale overvåkingssystemet for smittsomme sykdommer (MSIS)</a
    >.
  </p>
  <p>
    Under er er en oppsummering i tre kategorier:
    <strong style="color:{seriesColors[2]}"
      >seksuelt overførbare sykdommer</strong
    >,
    <strong style="color:{seriesColors[0]}">Covid-19</strong> og
    <strong
      class="pointer"
      style="color:{seriesColors[1]}"
      on:click={() => visDiag()}>restlige diagnosekategorier</strong
    >.
  </p>
  <p>
    <span>
      {#if viserDiag}
        <p>
          <strong
            >Restlige diagnosekategorier (Covid-19 isolert ut av kategorien
            "Alvorlige, systemiske sykdommer", hvorav de øvrige diagnosene vises
            individuelt):</strong
          >
        </p>
        <p>
          <a
            href="https://www.fhi.no/contentassets/88572059c1f34197ad1d07dd83ae63ed/msis_meldingskriterier_per_28042020.pdf"
            >Definisjon av hvilke sykdommer som registreres</a
          >.
        </p>
        <ul>
  ...
  <article class="chart">
    <div class="chart-container">
      ...
      >
        ...
        <Html>
          <!-- <Labels/> -->
          <SharedTooltip dataset={summedData} />
        </Html>
      </LayerCake>
    </div>
  </article>
