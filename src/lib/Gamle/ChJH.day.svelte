<script>
  ...
  $: lastRepShort = shavedData[shavedData.length - 1].date.toLocaleDateString(
    'no-NO',
    options
  )
...
<article class="text">
  <h2>
    ...
    {#if currAvg}
      ...
      {:else}
        <span class="sistedag"
          >{new Intl.NumberFormat('no-NO').format(currAvg)}</span
        >
      {/if}
    {:else}
  ...
  <p>
    <span>
      {#if range > 1}
        <span style="color: {textHighlightColor}"> Glidende gjennomsnitt</span>
        over {range} dager.
      {:else}
