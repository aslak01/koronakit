<script>
  ...
  $: mvAvgDtEnd =
    dataLong[0].values[dataLong[0].values.length - 1].date.toLocaleDateString(
      'no-NO'
    )
...
<article class="text">
  ...
  <p>
    Viser relativ mengde bevegelse med
    <strong
      class="modus"
      style="color:{seriesColors[0]}"
      on:click={() => selectHighlight('car')}>bil</strong
    >,
    <strong
      class="modus"
      style="color:{seriesColors[1]}"
      on:click={() => selectHighlight('bus')}>kollektivtransport</strong
    >
    og
    <strong
      class="modus"
      style="color:{seriesColors[2]}"
      on:click={() => selectHighlight('ped')}>til fots</strong
    >
    relativt til Apples anslag av normalnivået.
  </p>
  <p>
    <span
      >{#if range > 1}Glidende gjennomsnitt over {range} dager{:else}Alle tall{/if}
      mellom {mvAvgDtStart} og {mvAvgDtEnd}.</span
    >
  </p>
</article>
<section>
  <article class="topcontrols" style="text-align: center">
    ...
    />
    {#if highlightExists}
      <Close32 class="xbtn" on:click={() => selectHighlight('x')} />
    {/if}
  </article>
  <article class="chart">
      ...
      >
        ...
        <Html>
          <AxisX
            ...
            {xTickColor}
          />
          <AxisY ticks={4} gridlines={true} {yTickColor} />
        </Html>
        <Html>
          <SharedTooltip formatTitle={formatTickX} dataset={shavedMvAvg} />
        </Html>
      </LayerCake>
    </div>
  </article>
